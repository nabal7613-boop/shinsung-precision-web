...
document.addEventListener('DOMContentLoaded', () => {
  initHeader();
  initProductTuna2pcsBody();
});

function initProductTuna2pcsBody() {
  initHeroReveal();
  initSpotlightReveal();
  initHeroCellTabs();
  initDetailPanels();
...
function initHashTab() {
  const tabIndex = window.location.hash === '#drd-line' ? 1 : 0;
...
function initHeroVisualSync() {
  ...
  const labels = ['CNC SHEET FEEDER PRESS', 'DRD 2PCS CAN SHEET FEEDER PRESS LINE'];
