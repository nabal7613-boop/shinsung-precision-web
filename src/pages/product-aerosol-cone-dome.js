...
document.addEventListener('DOMContentLoaded', () => {
  initHeader();
  initProductAerosolConeDome();
});

function initProductAerosolConeDome() {
  initHeroReveal();
  initSpotlightReveal();
  initHeroCellTabs();
  initDetailPanels();
...
function initHashTab() {
  const tabIndex = window.location.hash === '#liner-dryer' ? 1 : 0;
  if (tabIndex === 0) return;

  const tab = document.querySelector(`.hero-cell-tab[data-index="${tabIndex}"]`);
  tab?.click();
}
...
function initHeroVisualSync() {
  ...
  const labels = ['AUTO TRANSFER PRESS', 'AUTO LINER & DRYER'];
